...
func (s *OpenAIGatewayService) listSchedulableAccounts(ctx context.Context, groupID *int64, platform string) ([]Account, error) {
	platform = NormalizeOpenAICompatiblePlatform(platform)
	if accounts, handled, err := temporaryDispatchPoolAccounts(ctx, s.accountRepo, groupID, platform); handled || err != nil {
		if err != nil {
			return nil, err
		}
...
func (s *OpenAIGatewayService) openAIAccountMatchesSchedulingGroup(ctx context.Context, account *Account, groupID *int64) bool {
	if account != nil {
		if allowed, active := temporaryDispatchAllowsAccount(ctx, groupID, account.ID); active {
			return allowed
		}
	}
