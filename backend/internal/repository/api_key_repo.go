	...
	m, err := r.activeQuery().
		...
		WithGroup(func(q *dbent.GroupQuery) {
			q.Select(
				...
				group.FieldTemporaryDispatchAccountIds,
				group.FieldTemporaryDispatchAccountDeadlines,
				group.FieldTemporaryDispatchID,
				group.FieldTemporaryDispatchStartedAt,
				group.FieldTemporaryDispatchExpiresAt,
				group.FieldTemporaryDispatchMode,
				group.FieldTemporaryDispatchUsageMetric,
				group.FieldTemporaryDispatchQuotaWindow,
				group.FieldTemporaryDispatchBaselinePercent,
				group.FieldTemporaryDispatchTargetPercent,
				group.FieldTemporaryDispatchCurrentPercent,
				group.FieldTemporaryDispatchQuotaResetAt,
			)
		}).
		Only(ctx)
	if err != nil {
...
func groupEntityToService(g *dbent.Group) *service.Group {
	...
	return &service.Group{
		...
		TemporaryDispatchAccountDeadlines: cloneTemporaryDispatchDeadlines(g.TemporaryDispatchAccountDeadlines),
		...
		TemporaryDispatchMode:             derefString(g.TemporaryDispatchMode),
		TemporaryDispatchUsageMetric:      derefString(g.TemporaryDispatchUsageMetric),
		TemporaryDispatchQuotaWindow:      derefString(g.TemporaryDispatchQuotaWindow),
		TemporaryDispatchBaselinePercent:  g.TemporaryDispatchBaselinePercent,
		TemporaryDispatchTargetPercent:    g.TemporaryDispatchTargetPercent,
		TemporaryDispatchCurrentPercent:   g.TemporaryDispatchCurrentPercent,
		TemporaryDispatchQuotaResetAt:     g.TemporaryDispatchQuotaResetAt,
...
func cloneTemporaryDispatchDeadlines(in map[string]time.Time) map[string]time.Time {
	if len(in) == 0 {
		return nil
	}
	out := make(map[string]time.Time, len(in))
	for accountID, deadline := range in {
		out[accountID] = deadline
	}
	return out
}
