...
type Group struct {
	...
	IsCarpool bool `json:"is_carpool,omitempty"`
...
func (_m *Group) assignValues(columns []string, values []any) error {
	...
	for i := range columns {
		...
		case group.FieldIsCarpool:
			if value, ok := values[i].(*sql.NullBool); !ok {
				return fmt.Errorf("unexpected type %T for field is_carpool", values[i])
			} else if value.Valid {
				_m.IsCarpool = value.Bool
			}
...
func (_m *Group) String() string {
	...
	builder.WriteString(", ")
	builder.WriteString("is_carpool=")
	builder.WriteString(fmt.Sprintf("%v", _m.IsCarpool))
	builder.WriteString(", ")
