export default {
    ...
    groups: {
      ...
      temporaryDispatch: {
        ...
        modeHybrid: '用量或时间',
        targetAccount: '目标账号',
        searchAccount: '搜索同平台账号...',
        searchMoreAccounts: '继续搜索账号...',
        selectedAccounts: '已选择 {count}/{max} 个账号；每个账号可独立设置结束条件',
        clearAccounts: '清空',
        removeAccount: '移除账号 {account}',
        accountLimit: '一次最多选择 {max} 个目标账号',
        incompatibleAccountsRemoved: '已移除 {count} 个不支持用量模式的账号',
        duration: '持续时间（分钟）',
        durationHint: '1–1440 分钟；到期后自动恢复原账号池',
        accountDurationHint: '1–1440 分钟；到期后仅此账号退出临时账号池',
        quotaWindow: '额度窗口（只能选择一个）',
        quotaTargetInput: '目标用量（%）',
        costWindow: '消费统计周期（只能选择一个）',
        targetCost: '目标用户消费金额（$）',
        targetCostHint: '累计本次调度分组中实际由该账号承接的用户最终消费，已包含分组及用户倍率。',
        quotaLoading: '正在获取账号最新额度…',
        quotaCurrent: '当前',
        quotaTarget: '目标',
        quotaReset: '重置',
        quotaUnavailable: '账号没有有效的 {window} 额度窗口',
        quotaLoadFailed: '获取账号额度失败',
        quotaTargetInvalid: '目标用量必须大于当前 {current}% 且不超过 100%',
        sharedQuotaHint: '账号不会独占；其他业务产生的配额用量也会计入结束进度。',
        userSpendHint: '只统计本次调度分组且实际由该目标账号承接的用户消费。',
        noAccounts: '没有匹配的可调度账号',
        active: '账号 #{account}',
        activePool: '临时账号池：{count} 个账号',
        expiresAt: '到期：{time}',
        start: '开始接管',
        started: '临时调度已启动',
        stopped: '已停止临时调度，分组已恢复原账号池',
        adjustAction: '调整临时调度',
        adjustTitle: '设置临时调度目标',
        adjustHint: '直接设置新的最终目标，当前进度不会清零。同一调度任务关联的所有分组会同步更新。',
        targetPercent: '目标用量（%）',
        targetSpend: '目标用户消费金额（$）',
        targetReachedHint: '当前进度已达到该目标，保存后此账号将立即退出临时账号池。',
        extendDuration: '延长时间（分钟）',
        currentProgress: '当前 {current} / 目标 {target}',
        adjusted: '临时调度目标已更新',
        adjustSubmit: '确认设置',
        selectGroups: '请先选择分组',
        samePlatform: '只能同时选择相同平台的分组',
        compositeUnsupported: '复合分组暂不支持临时切换到单一账号',
        selectAccount: '请选择目标账号',
        failed: '临时调度操作失败'
      },
      claudeMaxSimulation: {
