export default {
    accounts: {
      ...
      editAccount: '编辑账号',
      monthlyCost: '月成本',
      monthlyCostPlaceholder: '未配置',
      monthlyCostHint: '选填；用于使用记录中的成本系数估算，留空表示未配置。',
      monthlyCostInvalid: '月成本必须是大于 0 的有效金额',
      deleteAccount: '删除账号',
      searchAccounts: '搜索账号...',
      notes: '备注',
      ...
      openaiQuotaReset: {
        ...
        reset: '重置',
        carpoolReset: '重置拼车订阅',
        carpoolResetTitle: '重置拼车分组订阅配额',
        carpoolResetConfirm: '将重置此账号所绑定拼车分组中的 {count} 个有效订阅已配置的日/周/月配额。不会改变订阅状态和有效期。',
        carpoolResetSuccess: '已重置 {count} 个拼车订阅的配额',
        carpoolResetFailed: '账号额度重置成功，但同步重置拼车订阅配额失败。请使用旁边的手动兜底操作并检查审计记录。',
        carpoolResetTooltip: '手动重置该账号绑定的拼车分组中有效订阅配额',
        carpoolNoSubscriptions: '该账号没有绑定包含已配置配额的有效拼车订阅',
        carpoolUndo: '撤回上次重置',
        carpoolUndoTitle: '撤回最近一次拼车订阅配额重置',
        carpoolUndoConfirm: '恢复最近一次重置前的配额快照，同时保留重置后新增的用量。如果之后又发生过配额重置，为避免覆盖新状态，系统会拒绝撤回。',
        carpoolUndoSuccess: '已恢复 {count} 个订阅的配额，并保留重置后新增用量',
